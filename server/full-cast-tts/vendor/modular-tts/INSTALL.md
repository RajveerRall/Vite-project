...
This guide will help you install and use the `@your-scope/modular-tts` package in your other projects. The package now includes **Multi-User Support**, **Chat Thread Parser**, and **Comprehensive Audio Minute Tracking**.

## 🆕 What's Included (v0.1.0+)

- **Multi-User Concurrency** - Per-user CastingManager instances and session isolation
- **Chat Thread Parser** - Advanced two-step LLM process with context management
- **Audio Minute Tracking** - Complete analytics of all generated audio content
- **Enhanced Voice Management** - 30+ Kokoro voices with descriptions
- **User Identity Management** - X-User-Id headers and JWT token support
- **Comprehensive API** - 15+ endpoints for full system control
...
import { 
  ...
  Pipeline,
  PipelineOptions,
  chatThreadParser,  // NEW: Advanced parser with context management
  UsageTrackingService  // NEW: Audio minute tracking
...
// Each user gets their own CastingManager instance
const userCastingManagers = new Map<string, CastingManager>();

function getCastingManager(userId: string): CastingManager {
  if (!userCastingManagers.has(userId)) {
    userCastingManagers.set(userId, new CastingManager());
  }
...
app.post('/api/tts', (req, res) => {
  ...
  const userCastingManager = getCastingManager(userId);
  
  // Use user-specific casting manager
...
```typescript
import { ModularAIFactory, chatThreadParser } from '@your-scope/modular-tts';

const factory = new ModularAIFactory({
  gemini: { apiKey: process.env.GEMINI_API_KEY, model: 'gemini-2.0-flash' }
});

// Use the new chat thread parser with context management
const dialogueScript = await factory.createAndExecuteChain({
  llmIds: ['gemini-2.0-flash'],
  parserId: 'chatThreadParser',  // NEW: Advanced parser
  rawTextInput: 'Your text here',
  context: {
    chatHistory: [],  // Maintains conversation context
    CASTING_CONTEXT: userCastingManager.getCharacterMap(),
    AVAILABLE_VOICES: userCastingManager.getAvailableVoicesForLLM()
  }
});
```

### 🆕 Audio Minute Tracking

```typescript
import { UsageTrackingService } from '@your-scope/modular-tts';
...
const audioStats = usageTracker.getAudioMinuteStats();
console.log(`Total audio generated: ${audioStats.totalAudioMinutes} minutes`);
console.log(`By provider:`, audioStats.byProvider);
console.log(`Daily usage:`, audioStats.byDay);
```
