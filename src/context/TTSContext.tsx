...
import { useReaderSettingsContext } from './ReaderSettingsContext';
...
export const TTSProvider: React.FC<TTSProviderProps> = ({ children }) => {
    const { ttsSpeed } = useReaderSettingsContext();
    const [state, setState] = useState<TTSState>(() => {
        const initial = getInitialState();
        return { ...initial, playbackRate: ttsSpeed };
    });

    // Update playbackRate when global settings change
    React.useEffect(() => {
        setState(prev => ({ ...prev, playbackRate: ttsSpeed }));
    }, [ttsSpeed]);
