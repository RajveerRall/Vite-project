...
export const BookProvider: React.FC<BookProviderProps> = ({ children }) => {
  ...
  const { user, authInitialized, isAuthenticated, hasExplicitlySignedOut } = useAuth();
  ...
  } = useBookStorage(userId, authInitialized);
  ...
  useEffect(() => {
    // CRITICAL: Safety check. It prevents the app from saving an empty
    // book list when it first starts, before it has loaded your library.
    if (!isInitialLoadComplete) {
      console.log('[BookContext Save] Initial load not complete, blocking save');
      return;
    }
    ...
    if (!userId && books.length === 0) {
      console.log('[BookContext Save] Skipping empty save for guest user during cleanup');
      return;
    }

    // Prevent save/load loops: only save if books actually changed
    // We compare metadata that impacts sorting or persistence
    const booksKey = JSON.stringify(books.map(b => ({ 
      id: b.id, 
    ...
    // Verification: Ensure the books we are about to save actually belong to this user session
    // This is checked inside saveBooksToStorage (via useBookStorage), but we capture it here too
    console.log(`[BookContext Save] Preparing to save ${books.length} books for user context: ${userId || 'guest'}`);
    ...
    saveBooksToStorage(books, true)
      .then(() => {
        ...
        console.log(`[BookContext Save] Successfully persisted ${books.length} books`);
      })
      .catch(error => {
  ...
  }, [books, isInitialLoadComplete, authInitialized, isSyncingFromCloud, userId, saveBooksToStorage]);
