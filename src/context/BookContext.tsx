...
export const BookProvider: React.FC<BookProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    const loadBooksFromStorage = async () => {
      console.log("[LocalForage Load] Attempting to load books from 'bookStorage'.");
      setIsLoading(true); // Indicate global loading for app start
      try {
        const keys = await localforage.keys(); // Keys from "bookStorage"
        console.log("[LocalForage Load] Keys found in 'bookStorage':", keys);
        const bookMetadataKeys = keys.filter(key => key.startsWith('book_metadata_'));
        console.log("[LocalForage Load] Metadata keys to process:", bookMetadataKeys);

        const loadedBooks: BookData[] = [];
        if (bookMetadataKeys.length === 0) {
            console.log("[LocalForage Load] No book metadata keys found. Library is empty.");
        }

        for (const key of bookMetadataKeys) {
          const bookId = key.replace('book_metadata_', '');
          const metadata = await localforage.getItem(key) as BookData; // Assert type or validate
          const fileKey = `book_file_${bookId}`;
          const file = await localforage.getItem(fileKey) as File; // Assert type or validate

          if (metadata && file) {
            console.log(`[LocalForage Load] Loaded: ${metadata.title} (Page: ${metadata.currentPage})`);
            // Ensure all expected fields are present
            loadedBooks.push({
              ...metadata, // Spread all properties from stored metadata
              file: file, // Assign the loaded file
              // totalPages might not be stored or relevant to store, it's derived on openBook
            });
          } else {
            console.warn(`[LocalForage Load] Missing metadata or file for key: ${key}. Metadata: ${!!metadata}, File: ${!!file}`);
          }
        }
        setBooks(loadedBooks);
        console.log("[LocalForage Load] Finished loading books. Count:", loadedBooks.length);
      } catch (error) {
        console.error("[LocalForage Load] Error loading books from 'bookStorage'", error);
        setBooks([]); // Fallback to empty library on error
      } finally {
        setIsLoading(false);
        setIsInitialLoadComplete(true); // Signal that initial load attempt is done
      }
    };
  ...
  // 2. Save books to LocalForage whenever the `books` array changes, AFTER initial load.
  useEffect(() => {
    // Only save if the initial load from LocalForage is complete.
    // This prevents saving an empty `books` array during app initialization
    // before LocalForage has had a chance to populate it.
    if (!isInitialLoadComplete) {
      console.log('[LocalForage Save] SKIPPED: Initial load not yet complete.');
      return;
    }

    const saveBooksToStorage = async () => {
      console.log(`[LocalForage Save] SAVE FIRING. Books to save: ${books.length}`);
      try {
        // Clear only keys related to bookStorage for this app instance to avoid conflicts if DB name is shared
        // A more robust way is to manage a list of current book IDs and only remove/update those.
        // For now, this "clear and save all" is simpler if keys are well-prefixed.
        const allKeysInStore = await localforage.keys(); // Keys from "bookStorage"
        const bookKeysInStore = allKeysInStore.filter(
          key => key.startsWith('book_metadata_') || key.startsWith('book_file_')
        );

        // Get IDs of books currently in the state
        const currentBookIdsInState = new Set(books.map(b => b.id));

        // Remove keys from localForage that are no longer in the state
        for (const key of bookKeysInStore) {
          const isMetadataKey = key.startsWith('book_metadata_');
          const bookIdInKey = isMetadataKey ? key.replace('book_metadata_', '') : key.replace('book_file_', '');
          if (!currentBookIdsInState.has(bookIdInKey)) {
            console.log(`[LocalForage Save] Removing stale key: ${key}`);
            await localforage.removeItem(key);
          }
        }

        // Save all books currently in the state
        for (const book of books) {
          console.log(`[LocalForage Save] Saving to 'bookStorage': ${book.title} (ID: ${book.id}, Page: ${book.currentPage})`);
          await localforage.setItem(`book_metadata_${book.id}`, {
            // Explicitly list all properties to be saved from BookData
            id: book.id,
            title: book.title,
            author: book.author,
            coverUrl: book.coverUrl,
            currentPage: book.currentPage,
            totalPages: book.totalPages, // Save this if it's part of BookData and important
            lastRead: book.lastRead,
            lastChapter: book.lastChapter, // Make sure TOCItem is serializable
            // Do NOT save the 'file' object in metadata; it's saved separately
          });
          // The File object itself
          await localforage.setItem(`book_file_${book.id}`, book.file);
        }
        console.log('[LocalForage Save] SAVE COMPLETED to "bookStorage".');
      } catch (error) {
        console.error("[LocalForage Save] Error saving books to 'bookStorage'", error);
      }
    };

    saveBooksToStorage();
  }, [books, isInitialLoadComplete]); // Trigger save when `books` array changes or initial load completes
