...
export const LegacyTTSPlayer: React.FC = () => {
    const {
        currentBookId,
        currentBookTitle,
        currentBookAuthor,
    ...
    // Instances
    const playerInstanceId = useRef(`LegacyPlayer_${Date.now()}`).current;
    ...
    useEffect(() => {
        ...
        if (playbackStrategyRef.current) {
            try {
                // Full cleanup of previous strategy to ensure no queues/buffers leak
                playbackStrategyRef.current.stop();
                playbackStrategyRef.current.cleanup();
            } catch (e) {
                console.warn('Error cleanup strategy:', e);
            }
            // Re-initialize immediately with fresh state
            playbackStrategyRef.current = createAdaptivePlaybackStrategy({
                playbackRate,
                instanceId: playerInstanceId,
                forceStrategy: 'auto'
        ...
        const loadBook = async () => {
            ...
            try {
                ...
                console.error('[LegacyPlayer] Error loading book:', e);
    ...
    useEffect(() => {
        const loadChapter = async () => {
            ...
            if (chapterChanged && !chunkChanged) {
                console.log(`[LegacyPlayer] Natural transition from ${lastChapterIdRef.current} to ${currentChapterId}. Resetting index to 0.`);
                setTTSState({ currentChunkIndex: 0 });
            } else if (chapterChanged && chunkChanged) {
                console.log(`[LegacyPlayer] Explicit jump to ${currentChapterId} at chunk ${currentChunkIndex}.`);
            }
            ...
            if (previousChapterIdRef.current === currentChapterId && currentText) {
                console.log('[LegacyPlayer] Chapter already loaded, finalizing start');
            ...
            try {
                ...
                if (!targetHref) {
                    console.warn('[LegacyPlayer] Could not find spine item for chapter:', currentChapterId);
                    setTTSState({ isLoading: false });
                    return;
                }
                ...
                console.error('[LegacyPlayer] Error loading chapter:', e);
                setTTSState({ isLoading: false });
            }
        };
    ...
    const playChunk = useCallback(async (index: number) => {
        ...
        try {
            ...
            if (!source) {
                console.log(`[LegacyPlayer] Source missing for chunk ${index}, fetching...`);
                ...
                if (!source) {
                    console.log(`[LegacyPlayer] Source still missing for chunk ${index} after fetch, waiting 500ms...`);
            ...
            if (!source) {
                console.error(`[LegacyPlayer] CRITICAL: Failed to load source for chunk ${index}. Buffering failed or was cancelled.`);
            ...
            console.error('[LegacyPlayer] Playback error:', e);
            // Auto-pause on error?
        }
    ...
    useEffect(() => {
        ...
        if (isPlaying && !isPaused && chunks.length > 0) {
            ...
            if ((isNotPlaying || isPlayingWrongChunk) && !isDuplicateCall) {
                console.log(`[LegacyPlayer] Navigation/Start: chunk ${currentChunkIndex}. Strategy state: ${isNotPlaying ? 'stopped' : `on chunk ${strategyChunkIndex}`}`);
                lastPlayedChunkRef.current = currentChunkIndex;
                playChunk(currentChunkIndex);
            }
        } else {
            if (strategy.isPlaying()) {
                console.log(`[LegacyPlayer] Stopping playback as state is not active`);
                strategy.pause();
            }
            // Reset when stopped
